...
# Load the Excel file
file_path = 'data/results_ToM.xlsx'
...
pivot_df = df.groupby(['n_agents', 'm_examples']).agg(
    performance=('result', 'mean'),
    count=('result', 'count'),  # Count for standard error calculation
    std_dev=('result', 'std')  # Standard deviation for standard error
).reset_index()
